class PhotosController < ApplicationController
  ...
  def notify_subscribers(event, photo)
    # Собираем всех подписчиков кроме автора фотографии в массив мэйлов, исключаем повторяющиеся
    all_emails = ([event.user.email] + event.subscriptions.map(&:user_email) - [photo.user.email]).uniq
